...
public class SeckillOrderServiceImpl implements SeckillOrderService {
    ...
    public void submitOrder(Long id, String userId) {
        ...
        if(member){
            //查询用户是否有订单
            Object order = redisTemplate.boundHashOps(TbSeckillGoods.class.getSimpleName()).get(userId);
            //判断订单是否为空
            if(order!=null){
     ...
     */
    @Override
    public TbSeckillOrder getByUserId(String userId) {
     ...
     */
    @Override
    public void updateStatus(String userId, String out_trade_no, String status) {
        //查询处订单信息
        TbSeckillOrder seckillOrder = (TbSeckillOrder) redisTemplate.boundHashOps(TbSeckillOrder.class.getSimpleName()).get(userId);

        //判断订单是否存在
        if(seckillOrder == null || Long.valueOf(out_trade_no).longValue()!= seckillOrder.getId()){
        ...
        //清空用户排队信息
        redisTemplate.boundSetOps(SysConstants.SECKILL_USER_QUEUE).remove(userId);

        //删除redis缓存
        redisTemplate.boundHashOps(TbSeckillOrder.class.getSimpleName()).delete(userId);
    ...
    public void removeRedisOrder(String userId) {
        ...
        //恢复库存数量
        TbSeckillGoods seckillGoods = (TbSeckillGoods) redisTemplate.boundHashOps(TbSeckillGoods.class.getSimpleName()).get(seckillOrder.getSeckillId());
        //判断是否存在
