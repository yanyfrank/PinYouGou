...
public class CartServiceImpl implements CartService {
     ...
     * 2，判断商品是否存在
     * 3，判断商品是否启用
     * 4，判断新添加的商品是否属于原有购物车列表中某个商家（判断具有相同的商家）
     * 5，如果有相同商家，判断是否具有相同的商品
     * 6，如果有相同的商品，商品数量想家，总价格重新计算
     * 7，如果没有相同的商品，把新的商品直接添加此商家
     * 8，如果过没有相同的商家商品，新建一个商家对象，添加到购物车列表集合即可
     */
    @Override
    public List<Cart> addItemsToCartList(List<Cart> cartList, Long itemId, Integer num) {
        ...
        if(cart!=null){
            ...
            TbOrderItem orderItem = this.createOrderItem(item,itemId,num);
    ...
    private TbOrderItem isSameItem(List<TbOrderItem> orderItemList, Long itemId) {
        //循环遍历商家购物列表，判断是否具有相同的商品
        for (TbOrderItem orderItem : orderItemList) {
            //如果商品id和新添加的商品id相等，说明有相同的商品
